...
class ImageBasedCrossEntropyLoss2d(nn.Module):
    ...
    def calculate_weights(self, target):
        ...
        """
        hist = np.histogram(
            target.flatten(), range(self.num_classes + 1), normed=True)[0]
        if self.norm:
...
class ImgWtLossSoftNLL(nn.Module):
    ...
    def forward(self, inputs, target):
        ...
        if (cfg.REDUCE_BORDER_EPOCH != -1
                and cfg.EPOCH > cfg.REDUCE_BORDER_EPOCH) and self.ohem:
            return self.ohem_loss(inputs,
                                  self.onehot2label(target[:, :-1, :, :]))
        if self.fp16:
...
class OhemWithAux(nn.Module):
    ...
    def __init__(self,
        ...
        self.main_loss = OhemCrossEntropy2dTensor(ignore_index, thresh,
                                                  min_kept)
        self.aux_loss = OhemCrossEntropy2dTensor(ignore_index, thresh,
                                                 min_kept)

    def forward(self, pred, target):
        x_main, x_aux = pred
        return self.main_loss(
            x_main, target) + self.aux_weight * self.aux_loss(x_aux, target)
...
class OhemCrossEntropy2dTensor(nn.Module):
    ...
    def __init__(self,
        ...
        if use_weight:
            ...
            self.criterion = torch.nn.CrossEntropyLoss(
                reduction="elementwise_mean", ignore_index=ignore_index)
...
class JointEdgeSegLoss(nn.Module):
    ''' Loss used in `Improving Semantic Segmentation via Decoupled Body and Edge Supervision`.
    '''
    def __init__(self,
                 num_classes,
                 ignore_index=255,
        ...
        if mode == 'train':
            ...
            self.seg_loss = CrossEntropyLoss2d(
                size_average=True, ignore_index=ignore_index).cuda()
        elif mode == 'val':
            self.seg_loss = CrossEntropyLoss2d(
                size_average=True, ignore_index=ignore_index).cuda()

        self.seg_body_loss = ImgWtLossSoftNLL(
            classes=num_classes,
            ignore_index=ignore_index,
            upper_bound=1.0,
            ohem=False).cuda()
        # self.edge_ohem_loss = OhemCrossEntropy2dTensor(ignore_index=ignore_index, min_kept=5000).cuda()
        self.edge_ohem_loss = CrossEntropyLoss2d(
            ignore_index=ignore_index, size_average=True).cuda()
    ...
    def edge_attention(self, input, target, edge):
        ...
        edge_ohem_loss = self.edge_ohem_loss(
    ...
    def forward(self,
        ...
        losses['seg'] = self.loss_weight * self.seg_weight * self.seg_loss(
            seg_in, segmask)
        losses[
            'body'] = self.loss_weight * self.seg_body_weight * self.seg_body_loss(
                seg_body_in, seg_bord_mask)
        losses['edge'] = self.loss_weight * edge_weight * 20 * self.bce2d(
            edge_in, edgemask)
